                ...
                for n1 = 0:(am1-l1)
                    ...
                    for l2 = am2:-1:0
                        for n2 = 0:(am2-l2)
                            ...
                            out[index1+N1*(index2-1)] +=
                                prefac *
                                (Dx2 * Sy0 * Sz0 + Sx0 * Dy2 * Sz0 + Sx0 * Sy0 * Dz2)
                            index2 += 1
                        end
                    end
                    index1 += 1
...
@inline function generate_R_matrix!(
    R,
    fnbuf::FnBuffer,
    p::Float64,
    P,
...
function generate_quanta_list2(am2, am1, am, N1)
    dim =
        (am1 + 1)*(am1 + 2)*(am2 + 1)*(am2 + 2)*(am1+am2+3)*(
            am1^2+am2^2+4*am1*am2+9*(am1+am2)+20
        )÷240
...
@inline function generate_E_matrix!(E, maxam1, maxam2, P, A, B, a::Float64, b::Float64)
