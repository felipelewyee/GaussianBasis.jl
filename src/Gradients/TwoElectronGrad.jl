...
function ∇ERI_2e4c!(out, BS::BasisSet, iA)
    ...
    for (i, j, k, l) in unique_idx
        ...
        x_in_A = [x in Ashells for x in (i, j, k, l)]
    ...
    for ij in eachindex(ij_vals)
        ...
        for kl = ij:num_ij
            ...
            if x_in_A[1]
                ...
                bufy +=
                    reshape(buf[(Nijkl+1):(2*Nijkl)], Int(Ni), Int(Nj), Int(Nk), Int(Nl))
                bufz +=
                    reshape(buf[(2*Nijkl+1):(3*Nijkl)], Int(Ni), Int(Nj), Int(Nk), Int(Nl))
            end

            if x_in_A[2]
                cint2e_ip1_sph!(buf, [j, i, k, l], BS.lib)
                bufx += permutedims(
                    reshape(buf[1:Nijkl], Int(Nj), Int(Ni), Int(Nk), Int(Nl)),
                    (2, 1, 3, 4),
                )
                bufy += permutedims(
                    reshape(buf[(Nijkl+1):(2*Nijkl)], Int(Nj), Int(Ni), Int(Nk), Int(Nl)),
                    (2, 1, 3, 4),
                )
                bufz += permutedims(
                    reshape(buf[(2*Nijkl+1):(3*Nijkl)], Int(Nj), Int(Ni), Int(Nk), Int(Nl)),
                    (2, 1, 3, 4),
                )
            end
            ...
            if x_in_A[3]
                cint2e_ip1_sph!(buf, [k, l, i, j], BS.lib)
                bufx += permutedims(
                    reshape(buf[1:Nijkl], Int(Nk), Int(Nl), Int(Ni), Int(Nj)),
                    (3, 4, 1, 2),
                )
                bufy += permutedims(
                    reshape(buf[(Nijkl+1):(2*Nijkl)], Int(Nk), Int(Nl), Int(Ni), Int(Nj)),
                    (3, 4, 1, 2),
                )
                bufz += permutedims(
                    reshape(buf[(2*Nijkl+1):(3*Nijkl)], Int(Nk), Int(Nl), Int(Ni), Int(Nj)),
                    (3, 4, 1, 2),
                )
            end
            ...
            if x_in_A[4]
                cint2e_ip1_sph!(buf, [l, k, i, j], BS.lib)
                bufx += permutedims(
                    reshape(buf[1:Nijkl], Int(Nl), Int(Nk), Int(Ni), Int(Nj)),
                    (3, 4, 2, 1),
                )
                bufy += permutedims(
                    reshape(buf[(Nijkl+1):(2*Nijkl)], Int(Nl), Int(Nk), Int(Ni), Int(Nj)),
                    (3, 4, 2, 1),
                )
                bufz += permutedims(
                    reshape(buf[(2*Nijkl+1):(3*Nijkl)], Int(Nl), Int(Nk), Int(Ni), Int(Nj)),
                    (3, 4, 2, 1),
                )
            end
...
function ∇ERI_2e2c!(out, BS::BasisSet, iA)
    ...
    for i = 1:BS.nshells
        for j = i:BS.nshells # i <= j
            ...
            x_in_A = [x in Ashells for x in (i, j)]
